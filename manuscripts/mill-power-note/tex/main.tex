...
\newcommand{\versiondoi}{10.5281/zenodo.22834636}
\newcommand{\conceptdoi}{10.5281/zenodo.21511525}
\newcommand{\docrepo}{github.com/fsantibanezleal/CAOS\_MillDEM}
...
CAOS open-research programme \quad$\cdot$\quad CAOS\_MillDEM, the \texttt{milldem} tumbling-mill DEM package\\[2pt]
...
\thanks{Package \texttt{milldem} on PyPI (\url{https://pypi.org/project/milldem});
source code (MIT) at \url{https://github.com/fsantibanezleal/CAOS_MillDEM}.}}

\markboth{\doctype\ \docversion, 2026 \textperiodcentered\ CAOS open-research programme}%
{Santib\'a\~nez-Leal: milldem, a cross-platform soft-sphere DEM for tumbling-mill charge motion and power}
...
its power-to-model ratio drifts systematically with mill diameter, whereas the
...
DEM-to-model ratio in $[0.77,1.26]$, mean $1.06$) and keeps the size ratio within a factor of $1.51$. With the
small particle budget used, the ratio scatters by about $\pm0.2$: the agreement is a band around a classical
analytical model, not a fit to plant measurements.
\end{abstract}
...
NumPy, with an optional Numba~\cite{numba} just-in-time (JIT) compilation of the contact loop for a $10$--$50\times$ speed-up
...
We diagnose the cause of this drift instead of absorbing it into a fitted correction, and remove it with a
\emph{thin-three-dimensional slab}: an axial
...
(Section~\ref{sec:model}), the two routes and the slab rationale (Section~\ref{sec:routes}), the validation and
its residual scatter (Section~\ref{sec:validation}), and the architecture, limits and availability
(Sections~\ref{sec:arch}--\ref{sec:limits}). The reported agreement is not the result of fitting the engine to
the reference model.
...
friction on non-Linux machines is a significant cost, and a GPU cluster is overkill for a thin-slab power estimate.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.98\textwidth]{fig-slab.pdf}
...
torque by the mill length over the slab thickness. This is the route validated in Section~\ref{sec:validation}.

\section{Validation}\label{sec:validation}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.78\textwidth]{fig-validation.pdf}
\caption{Thin-3D-slab net power (blue) against the classical Hogg--Fuerstenau model (orange), from the
reproducible sweep of Table~\ref{tab:sweep}. \textbf{(a)} Size sweep at fixed fill $J=0.30$: across
mill diameters $3$--$6$~m the DEM power tracks the model, with DEM-to-model ratios $1.04, 1.16, 0.90, 0.77$
(numbers above each pair). \textbf{(b)} Fill sweep at $D=4$~m: net power rises monotonically with fill through
$J=0.35$ and tracks the model (ratios $1.04$--$1.26$), with the expected approach to the model's roll-off near
$J\!\approx\!0.47$. All eight configurations lie within the validated band; the residual scatter is discussed in
Section~\ref{sec:validation}.}
\label{fig:validation}
\end{figure*}

The central claim is that the thin-3D-slab net power agrees with a classical mill-power model and does so
...
model over four mill diameters at fixed fill and four fills at fixed diameter, each a full DEM simulation of
$1.3$~s of simulated time. Two results stand out. First, \emph{the DEM power stays within the validated band of the
classical model}: all eight configurations have a DEM-to-model ratio in $[0.77, 1.26]$, mean $1.06$. Second,
\emph{the ratio is size-consistent to within a factor of $1.51$}: across the $2\times$ diameter range the size
sweep ratios are $1.04, 1.16, 0.90, 0.77$ (spread $0.39$), without the systematic size drift of a single 2D
disc slice described in Section~\ref{sec:routes}. The fill sweep confirms that power rises monotonically with charge through
...
validated band; this table and Fig.~\ref{fig:validation} are generated from the archived sweep artifact.}
\label{tab:sweep}
\footnotesize
...
The agreement is a \emph{band}, not a tight fit: with a
...
p = simulate_power(MillConfig(diameter_m=5.0,
        phi_c=0.75, fill=0.30,
        ball_diameter_m=0.10))
print(p["net_power_kw"], p["arm_m"])
\end{lstlisting}

\begin{lstlisting}[language=bash]
milldem power --D 5 --phi 0.75 --J 0.30 \
    --ball 0.10 --L 6
\end{lstlisting}
...
tests, the validation statement (\texttt{docs/VALIDATION.md}), and the reproducible sweep and figure scripts are
available at \url{https://github.com/fsantibanezleal/CAOS_MillDEM}. The sweep artifact and both schematic
figures regenerate with no network access.

\section{Limitations}\label{sec:limits}

The 2D disc route is qualitative for power (it is a shape and regime tool). The thin-3D slab
...
measured values. The validation document in the repository states each of these limits, and the tests assert
...
systematic size drift of a 2D disc slice. The result is a reproducible mill DEM, validated against a
classical power model, that a course, a prototype or a CI job can install with \texttt{pip} and use within its
stated band.
...
The validation sweep is \path{manuscripts/mill-power-note/figures/validation_sweep.py}, which runs
...
\path{data/validation_sweep.json}. The figure script reads that artifact; the two schematics are hand-authored
theme-aware vector drawings. Installing \texttt{milldem[jit]} and running the sweep reproduces the table and
Fig.~\ref{fig:validation} exactly (the engine is seeded); the residual scatter reported above is the
run-to-run variation of a small-$N$, short-integration slab, not a fitted result.\par}
