"""``milldem`` command-line interface: run a mill DEM simulation and print/emit the settled metrics.
...
from . import ContactModel, MillConfig, MillDEM, __version__, compute_metrics


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="milldem", description="2D soft-sphere tumbling-mill DEM.")
    p.add_argument("--version", action="version", version=f"milldem {__version__}")
    sub = p.add_subparsers(dest="cmd", required=True)

    r = sub.add_parser("run", help="run a simulation and report the settled metrics")
